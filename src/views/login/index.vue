<template>
  <div class="container">
    <van-nav-bar title="登入" left-arrow @click-left="$router.back()"></van-nav-bar>
    <!-- shouji -->
    <van-cell-group>
      <van-field
        @blur="checkMobile"
    ...
    async login () {
      if (this.checkMobile() && this.checkCode()) {
        ...
        // 提示消息 表示登录成功
          ...
          let { redirectUrl } = this.$route.query // ?? 为什么用对象形式{ redirectUrl }   解构当前的路由信息
          if (redirectUrl) {
            this.$router.push('/')
          } else {
            this.$router.push('/')
          ...
          this.$gnotify({ type: 'danger', message: '登录失败' })
        }
      }
    },
